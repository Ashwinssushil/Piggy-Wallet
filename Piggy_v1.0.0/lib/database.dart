...
class DatabaseHelper {
  ...
  Future<Database> _initDB() async {
    ...
    return await openDatabase(
      ...
      onUpgrade: (db, oldVersion, newVersion) async {
        if (oldVersion < 2) {
          // Add indexes if upgrading from version 1
          await db.execute(
  ...
  Future<int> importFromCSV(String filePath) async {
    ...
    // Use a batch for faster inserts
    var batch = db.batch();
    final batchSize = 100; // Process in batches of 100
    ...
    // If we couldn't find the required columns, try to guess based on position
    if (titleIndex == -1 || amountIndex == -1 || typeIndex == -1 || dateIndex == -1) {
      // Assume standard format: ID,Title,Amount,Type,Date
    ...
    for (int i = 1; i < lines.length; i++) {
      ...
      try {
        ...
        if (typeIndex >= 0 && typeIndex < values.length) {
          ...
          // If type is not specified, try to determine from amount
  ...
  Future<String> generateTransactionReport([String currency = '₹']) async {
    // Convert currency symbols to PDF-friendly text
    String pdfCurrency;
    switch (currency) {
      case '₹':
        pdfCurrency = 'Rs. ';
    ...
    // Get transactions already sorted by date (newest first) from getTransactions()
    final transactions = await getTransactions();
